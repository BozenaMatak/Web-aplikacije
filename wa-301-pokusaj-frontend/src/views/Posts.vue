...
export default {
  ...
  created() {
    this.search() //tu sam pozvala onu search odozdo
    this.fetchPosts() //nisam sigurna da li ovo treba ovdje ako ju pozivam u search, ali probala sam oboje pa nije radilo
  },
  name: "posts",
  methods: {
    fetchPosts(term, title) {
      term = term || store.searchTerm
      fetch(`http://localhost:3000/posts?${title}=${term}`) //iskreno nisam ovo ovako nikad pisala pa ne znam ni što točno znači
        .then(response => {
   ...
   search(title, createdBy){ //kad pokrenem program ispada kao da se search uopće ne pokreće
     console.log(title); //mislim da mi ovdje ne prepoznaje što je uopće title jer kad u tražilicu napišem bilo što u konzoli mi se samo ispišu stva 3 posta
     if(!title){
